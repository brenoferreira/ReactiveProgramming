...
class EpidemySuite extends FunSuite {
  ...
  test("move within 5 days"){
    var movedTimes = 0
    for(i <- 0 to 100){
      val es = new EpidemySimulator
      val healthyPerson = (es.persons find {p => !p.infected}).get

      val c1 = (healthyPerson.row, healthyPerson.col)
      ...
      while(es.agenda.head.time < 6){
        es.next
        c2 = (healthyPerson.row, healthyPerson.col)

        movedTimes = movedTimes + (if(c2 == c1) 0 else 1)
      }

      //assert(healthyPerson.dead === false, "not dead")
      assert(movedTimes != 0, "A person should move within five days")
    }
  }
}