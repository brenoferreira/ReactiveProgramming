  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] =
      Observable[String]((observer:Observer[String]) => {
        val eventSubscription:Reaction = {
          case ValueChanged(e) => observer.onNext(e.text);
        }

        field.subscribe(eventSubscription)

        Subscription({
          field.unsubscribe(eventSubscription)
        })
      })
  }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] =
      Observable[Button]((observer:Observer[Button]) => {
        ...
        button.subscribe(eventSubscription)

        Subscription({
          button.unsubscribe(eventSubscription)
        })
      })
